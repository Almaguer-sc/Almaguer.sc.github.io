document.addEventListener('DOMContentLoaded', () => {
    ...
    <section class="section">
      <div class="container grid-12">
        ...
        <div style="grid-column: span 6;">
          ...
          <button class="btn btn-primary" id="btn-agregar-carrito" style="width:100%; margin-top: var(--space-2);">Agregar al carrito</button>
  ...
  // Selector de presentación — actualiza el precio mostrado y el tamaño seleccionado
  let tamanoSeleccionado = '250g';
  contenedor.querySelectorAll('.size-selector .size-option').forEach((boton) => {
    boton.addEventListener('click', () => {
      ...
      tamanoSeleccionado = boton.dataset.size;
      const precio = producto.precios[tamanoSeleccionado];
  ...
  botonAgregar.addEventListener('click', () => {
    almaguerAgregarAlCarrito({
      slug: producto.slug,
      nombre: producto.nombre,
      size: tamanoSeleccionado,
      precio: producto.precios[tamanoSeleccionado],
      foto: producto.fotos[0],
      cantidad: 1
    });
    const enlaceCarrito = document.getElementById('enlace-carrito');
    if (enlaceCarrito) enlaceCarrito.textContent = `Carrito (${almaguerContarItems()})`;
    const textoOriginal = botonAgregar.textContent;
    botonAgregar.textContent = 'Agregado ✓';
    setTimeout(() => { botonAgregar.textContent = textoOriginal; }, 1500);
  });
});
